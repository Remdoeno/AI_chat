...
LOG_DIR=${LOG_DIR:-wangcai_ai/logs}
PID_FILE=${PID_FILE:-wangcai_ai/wangcai_embedding.pid}
LOG_FILE="$LOG_DIR/wangcai_embedding_${EMBED_PORT}_$(date +%Y%m%d_%H%M%S).log"
...
if [ -f "$PID_FILE" ]; then
  ...
  if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" >/dev/null 2>&1; then
    echo "ERROR: wangcai embedding service already running with PID $OLD_PID"
    exit 1
  fi
  rm -f "$PID_FILE"
fi
...
echo "Starting Wangcai embedding service on ${EMBED_HOST}:${EMBED_PORT}"
...
if ! kill -0 "$PID" >/dev/null 2>&1; then
  echo "ERROR: wangcai embedding service failed to start. Last log lines:"
  tail -n 120 "$LOG_FILE" || true
  rm -f "$PID_FILE"
  exit 1
fi

echo "wangcai embedding service started with PID $PID"
