...
window.ScheduleConnections = (() => {
  ...
  function draw(grid) {
    ...
    groups.forEach(cards => {
      cards.sort((a,b) => Number(a.dataset.stageIndex)-Number(b.dataset.stageIndex));
      for (let i=1;i<cards.length;i++) {
        ...
        let d;
        if (cards[i-1].closest('.day-cell') === cards[i].closest('.day-cell')) {
          ...
          const cell=cards[i-1].closest('.day-cell').getBoundingClientRect();
