...
IMAGE_STATUS_TIMEOUT = float(os.environ.get("QWEN_IMAGE_STATUS_TIMEOUT", "3"))
IMAGE_GENERATION_TIMEOUT = float(os.environ.get("QWEN_IMAGE_GENERATION_TIMEOUT", "300"))
IMAGE_PROMPT_TIMEOUT = float(os.environ.get("QWEN_IMAGE_PROMPT_TIMEOUT", "45"))
HIDREAM_DEFAULT_IMAGE_SIZE = int(os.environ.get("QWEN_HIDREAM_IMAGE_SIZE", "1024"))
...
def request_hidream_images(prompt: str, negative_prompt: str, aspect_ratio: str, count: int) -> List[bytes]:
    ...
    with model_http_client(slot, timeout=IMAGE_GENERATION_TIMEOUT) as client:
        try:
            return request_hidream_flask_images(client, base_url, prompt, aspect_ratio, count)[:count]
...
def generate_image_batch(
    ...
    image_bytes_list = request_hidream_images(optimized_prompt, negative_prompt, aspect_ratio, int(count))
