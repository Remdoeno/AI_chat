# Local Qwen service orchestration. Keep process launching isolated from routes.
import socket
import subprocess


LOCAL_QWEN_PORT_CANDIDATES = [
    int(port.strip())
    for port in os.environ.get("QWEN_LOCAL_MODEL_PORTS", "8000").split(",")
    if port.strip().isdigit()
]
LOCAL_EMBEDDING_PORT_CANDIDATES = [
    int(port.strip())
    for port in os.environ.get("QWEN_LOCAL_EMBEDDING_PORTS", "8001").split(",")
    if port.strip().isdigit()
]
LOCAL_IMAGE_PORT_CANDIDATES = [
    int(port.strip())
    for port in os.environ.get("QWEN_LOCAL_IMAGE_PORTS", "8002").split(",")
    if port.strip().isdigit()
]
...
LOCAL_QWEN_START_SCRIPT = local_service_script_path(
    "QWEN_LOCAL_MODEL_START_SCRIPT",
    [
...
LOCAL_EMBEDDING_START_SCRIPT = local_service_script_path(
    "QWEN_LOCAL_EMBEDDING_START_SCRIPT",
    [
        APP_DIR.parent / "start_qwen_embedding.sh",
        APP_DIR.parent.parent / "start_qwen_embedding.sh",
    ],
)
LOCAL_IMAGE_START_SCRIPT = local_service_script_path(
    "QWEN_LOCAL_IMAGE_START_SCRIPT",
    [
...
def local_model_service_summary(qwen_status: Dict[str, object], embedding_status: Dict[str, object], image_status: Optional[Dict[str, object]] = None) -> str:
    qwen_running = bool(qwen_status.get("running"))
    embedding_running = bool(embedding_status.get("running"))
    image_running = bool((image_status or {}).get("running"))
    if qwen_running and embedding_running and image_running:
        return f"已运行：Qwen {qwen_status.get('port')} / Embedding {embedding_status.get('port')} / 画图 {image_status.get('port')}"
    missing = []
    if not qwen_running:
        missing.append("Qwen")
...
def local_model_service_status() -> Dict[str, object]:
    qwen_status = local_service_component_status(
        "qwen",
        LOCAL_QWEN_PORT_CANDIDATES,
        MODEL_NAME,
    )
    ...
    return {
        ...
        "image": image_status,
        "summary": local_model_service_summary(qwen_status, embedding_status, image_status),
    }
...
def select_local_service_gpus() -> Dict[str, str]:
    ...
    if len(gpus) >= 3:
        ...
        qwen_gpus = sorted([ordered[1][0], ordered[2][0]])
        return {"embedding": str(embedding_gpu), "qwen": ",".join(str(gpu) for gpu in qwen_gpus)}
    return {"embedding": "5", "qwen": "6,7"}
...
def start_missing_local_model_services(status: Dict[str, object]) -> Dict[str, object]:
    ...
    if not embedding_status.get("running"):
        ...
        command = (
            ...
            f"PID_FILE={web_dir_name}/qwen_embedding.pid "
            f"{LOCAL_EMBEDDING_START_SCRIPT}"
        )
        launch_shell_command(command, project_dir)
        commands.append(f"embedding:{embedding_port}")

    if not qwen_status.get("running"):
        qwen_port = int(qwen_status.get("port") or LOCAL_QWEN_PORT_CANDIDATES[0])
        if not port_is_free(qwen_port):
            raise RuntimeError(f"Qwen 端口 {qwen_port} 已被非目标服务占用")
        if not LOCAL_QWEN_START_SCRIPT.exists():
            raise RuntimeError(f"Qwen 启动脚本不存在: {LOCAL_QWEN_START_SCRIPT}")
        qwen_log = log_dir / f"start_qwen36_{qwen_port}_{int(time.time())}.launcher.log"
        command = (
            "nohup env "
            "KILL_OLD=0 "
            "INTERACTIVE=0 "
            f"GPUS={gpu_selection['qwen']} "
            "TP_SIZE=2 "
            "ENABLE_COT=0 "
            f"PORT={qwen_port} "
            f"{LOCAL_QWEN_START_SCRIPT} > {qwen_log} 2>&1 &"
        )
        launch_shell_command(command, project_dir)
        commands.append(f"qwen:{qwen_port}")
...
def apply_local_model_service_settings(status: Dict[str, object]) -> bool:
    ...
    embedding_status = status.get("embedding") if isinstance(status.get("embedding"), dict) else {}
    image_status = status.get("image") if isinstance(status.get("image"), dict) else {}
    has_core_services = bool(qwen_status.get("running") and embedding_status.get("running"))
    ...
    if has_core_services:
        qwen_base_url = str(qwen_status.get("base_url") or local_openai_base_url(8000)).rstrip("/")
