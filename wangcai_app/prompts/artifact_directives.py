ARTIFACT_THEATER_CONTEXT = (
    ...
    "用户可以通过聊天给这个小剧场下导演指令，例如建议某个固定角色后续有机会出场、少写某些旧角色、改变剧情方向、指定系列或风格。"
...
ARTIFACT_DIRECTIVE_AGENT_SYSTEM_PROMPT = (
    ...
    "- 用户可以用自然语言引导成果：建议角色出场、减少旧角色、调整剧情方向、指定系列、指定风格、安排角色关系或画面重点。\n"
    "- 这些指令是后台创作的导演参考，不是每篇必须执行的硬约束；除非用户明确说下一篇/立刻/必须，否则长期角色指令只表示可用素材。\n\n"
    "# 触发条件\n"
    ...
    "- priority 1 到 100，表示参考强度，不表示必然执行；“下一篇/必须/别再”通常更高，“偶尔/时不时”中等。\n"
...
ARTIFACT_DIRECTIVE_CONTEXT_HEADER = (
    ...
    "只有与本轮创作 prompt、被选系列或 next_artifact 直接相关的指令才会进入本段。"
    "写成果时应把它们当作候选方向，而不是每篇必须执行的硬性点名表。"
    "如果多个指令会导致角色堆叠、剧情重复或动机牵强，优先执行本轮创作 prompt，并保证作品自然因果。"
)
