# Generated by splitting app.py lines 501-1682. Keep execution order in wangcai_app/startup/loader.py.
...
def build_memory_retrieval_query(
    ...
    client, http_client, model_slot = openai_client_for_slot(
        ...
        timeout=MEMORY_QUERY_PLANNER_TIMEOUT,
    )
    try:
...
def parse_memory_gate_response(text: str) -> Dict[str, object]:
    ...
    return {
        ...
        "needs_self_profile": bool(parsed.get("needs_self_profile")),
        "reason": str(parsed.get("reason") or "").strip(),
    }


def fallback_memory_gate_decision() -> Dict[str, object]:
    return {"needs_memory": False, "needs_self_profile": False, "reason": "memory gate unavailable"}


def normalize_memory_gate_decision(value: object) -> Dict[str, object]:
    if isinstance(value, dict):
        return {
            "needs_memory": bool(value.get("needs_memory")),
            "needs_self_profile": bool(value.get("needs_self_profile")),
            "reason": str(value.get("reason") or "").strip(),
        }
    if value is None:
        return fallback_memory_gate_decision()
    return {"needs_memory": bool(value), "needs_self_profile": False, "reason": ""}
...
def build_memory_gate_user_prompt(
    ...
    return (
        f"{current_date_context()}\n\n"
        f"{context_block}"
        "请判断下面用户消息是否需要调用长期记忆，以及是否需要注入旺财系统自我认知档案。"
        "如果当前消息承接上文，包含“他、这件事、那我、刚才那个、它、这个”等指代，"
        "必须结合最近会话上下文判断是否需要回忆；不要只看当前一句。"
        "系统里存在 characters 类型的全局角色记忆；当用户询问旺财成果、小剧场、角色库、固定角色、登场角色，"
        "或询问某个可能是系统内角色的人物设定、形象、性格、背景、关系或剧情线索时，"
        "应设置 needs_memory=true，不要直接假定是外部百科人物。"
        "只有当用户在问助手/旺财系统自身、版本、功能、升级历史、记忆机制或模型配置时，"
        "才设置 needs_self_profile=true。\n\n"
        f"用户消息：{user_message}"
    )


def get_memory_gate_decision(
    user_message: str,
    session_id: str = "",
    visitor_ip: str = "unknown",
    analysis_trace_id: str = "",
    ...
    fallback = fallback_memory_gate_decision()
    messages = [
...
def should_use_memory_recall(
    ...
    decision = get_memory_gate_decision(
        user_message,
        session_id=session_id,
        visitor_ip=visitor_ip,
...
_DEFAULT_SHOULD_USE_MEMORY_RECALL = should_use_memory_recall


def resolve_memory_gate_decision(
    user_message: str,
    session_id: str = "",
    visitor_ip: str = "unknown",
    analysis_trace_id: str = "",
    ...
    if should_use_memory_recall is _DEFAULT_SHOULD_USE_MEMORY_RECALL:
        return get_memory_gate_decision(
            user_message,
            session_id=session_id,
            visitor_ip=visitor_ip,
            analysis_trace_id=analysis_trace_id,
    ...
    return normalize_memory_gate_decision(
        should_use_memory_recall(
            user_message,
            session_id=session_id,
            visitor_ip=visitor_ip,
            analysis_trace_id=analysis_trace_id,
...
def build_search_plan(
    ...
    client, http_client, model_slot = openai_client_for_slot(
        MODEL_SLOT_BACKGROUND,
        timeout=WEB_SEARCH_PLANNER_TIMEOUT,
    )
    try:
        resp = client.chat.completions.create(
            **model_completion_kwargs(model_slot),
...
LATE_NIGHT_TOMORROW_CONFIRM_BEFORE_HOUR = int(
    os.environ.get("WANGCAI_LATE_NIGHT_TOMORROW_CONFIRM_BEFORE_HOUR", "3")
)
