...
def compact_character_profile(profile: Dict[str, object], max_prompt_chars: int = 900) -> str:
    canonical_name = str(profile.get("canonical_name") or "未命名角色").strip() or "未命名角色"
    lines = [f"[固定角色 #{profile.get('id')}] {canonical_name}"]
    aliases = [str(item) for item in profile.get("aliases", []) if str(item).strip()]
    if aliases:
    ...
    lines.append(
        f"成果出场锚点：{canonical_name} 可以不出场；但一旦出场，必须保留以上形象、性格、背景和关系核心，不能只借名字另造角色。"
    )
    return "\n".join(lines)
...
def format_hidden_character_context_for_artifacts(
    ...
    def recent_mentions(profile: Dict[str, object]) -> int:
    ...
    def prompt_mentions(profile: Dict[str, object]) -> int:
        if not focus:
            return 0
        names = [str(profile.get("canonical_name") or ""), *[str(item) for item in profile.get("aliases", [])]]
        count = 0
        for name in names:
            ...
            if not text:
                continue
            count += focus.count(text)
        return count

    ranked_profiles = sorted(
        profiles,
        key=lambda item: (
            -prompt_mentions(item),
            recent_mentions(item),
            -int(item.get("id") or 0),
        ),
    )
    lines = [
        ARTIFACT_CHARACTER_CONTEXT_HEADER,
        "recent_mentions 表示近 12 个成果里大致出现次数；优先选择 recent_mentions 低且适合本篇目标的角色，避免同一批角色连续刷屏。",
        "prompt_mentions 表示本轮成果创作 prompt 是否点名该角色；如果 prompt_mentions 大于 0，应优先考虑这个角色，并严格保留其角色卡特征。",
        "以下角色资料用于角色保真，不等于强制出场名单；如果作品选择某个角色，必须按对应角色卡保留其核心特征。",
        "",
    ]
    for profile in ranked_profiles[:max_rows]:
        if focus:
            lines.append(f"本轮 prompt 点名次数：{prompt_mentions(profile)}")
        lines.append(f"近作出现次数：{recent_mentions(profile)}")
        lines.append(compact_character_profile(profile, max_prompt_chars=1100))
        lines.append("")
    return "\n".join(lines).strip()
