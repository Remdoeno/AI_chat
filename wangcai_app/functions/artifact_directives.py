...
def directive_recent_mention_count(item: Dict[str, object], recent_text: str) -> int:
    names = [str(item.get("subject") or ""), *[str(name) for name in item.get("characters", [])]]
    count = 0
    for name in names:
        text = name.strip()
        if not text:
            continue
        count += recent_text.count(text)
    return count


def should_inject_artifact_directive(item: Dict[str, object], recent_text: str) -> bool:
    if str(item.get("scope") or "") == "next_artifact":
        return True
    directive_type = str(item.get("directive_type") or "")
    if directive_type == "character_include":
        return False
    if directive_type != "character_include":
        return True
    return False
...
def artifact_directive_focus_terms(item: Dict[str, object]) -> List[str]:
    terms: List[str] = []
    for value in (
        item.get("subject"),
        ...
        *(item.get("characters") or []),
        ...
        text = clean_artifact_directive_text(value, 120)
        if text:
            terms.append(text)
    return terms


def artifact_directive_matches_focus(
    ...
    clean_series = clean_artifact_directive_text(series_title, 160)
    item_series = clean_artifact_directive_text(item.get("series_title"), 160)
    if clean_series and item_series and clean_series == item_series:
        return True
    for term in artifact_directive_focus_terms(item):
        if term and (term in focus or (clean_series and term == clean_series)):
            return True
    directive_type = str(item.get("directive_type") or "")
    has_subject = bool(artifact_directive_focus_terms(item))
    priority = int(item.get("priority") or 0)
    # Broad, subject-less high-priority style/image rules may still be relevant,
    # but ordinary persistent character/plot snippets should not drift into every artifact.
    return (
        not has_subject
        and priority >= 90
        and directive_type in {"style_rule", "image_rule", "series_rule", "other"}
    )
...
def format_hidden_artifact_directives_for_artifacts(
    ...
    directives = [
        item
        for item in directives
        if artifact_directive_matches_focus(item, focus_text=focus_text, series_title=series_title)
    ]
    if not directives:
        return ""
    ...
    if elevated:
        lines.append("本轮可优先参考的导演指令：")
    else:
        lines.append("本轮没有必须优先参考的导演指令；请以用户配置 prompt、题材变化、人物动机和作品质量为核心。")
    ordered_items = [*elevated, *reference]
    if reference:
        lines.append("其余导演指令只作为候选素材；不要因为这里出现某个角色，就让他/她强行出场。")
    for index, item in enumerate(ordered_items, start=1):
        mention_count = directive_recent_mention_count(item, recent_text)
        reference_strength = "优先参考" if item in elevated else "候选参考"
        if item.get("directive_type") == "character_include" and mention_count > 0:
            reference_strength = f"候选参考，近作已出现 {mention_count} 次，默认不强制使用"
        parts = [
            f"[导演指令 {index}] {reference_strength}",
