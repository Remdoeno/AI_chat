...
def idle_series_candidate_titles(custom_prompt: str, catalog: List[Dict[str, object]]) -> List[str]:
    ...
    def add(title: str) -> None:
        clean = normalize_idle_artifact_terms(title).strip()
    ...
    for item in catalog:
        ...
        if not title:
            continue
        if title in focus or any(quoted and (quoted in title or title in quoted) for quoted in quoted_titles):
            add(title)
...
def load_complete_idle_series_works(series_titles: List[str]) -> Dict[str, List[Dict[str, object]]]:
    ...
    clean_titles = [normalize_idle_artifact_terms(item).strip() for item in series_titles if str(item or "").strip()]
    if not clean_titles:
        return result
    with connect_db() as conn:
