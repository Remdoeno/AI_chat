...
# Generated by splitting app.py lines 9409-10128. Keep execution order in wangcai_app/startup/loader.py.
...
def build_system_prompt(
    ...
    precomputed_memory_gate: Optional[object] = None,
    ...
    if not web_search_context:
        ...
        try:
            if precomputed_memory_gate is None:
                memory_gate_decision = resolve_memory_gate_decision(
                    user_message,
                    session_id=session_id,
            ...
            use_memory = bool(memory_gate_decision.get("needs_memory"))
            if memory_debug is not None:
                memory_debug["memory_gate"] = "run" if use_memory else "skipped"
                memory_debug["self_profile"] = "run" if memory_gate_decision.get("needs_self_profile") else "skipped"
            if use_memory:
    ...
    if memory_gate_decision.get("needs_self_profile"):
...
def extract_delta_content(delta: object) -> str:
    ...
    if content:
        return normalize_model_text_content(content)
...
def normalize_model_text_content(content: object) -> str:
...
def completion_message_text(message: object) -> str:
    content = getattr(message, "content", None)
    text = normalize_model_text_content(content)
    if text.strip():
        return text
    ...
    if isinstance(model_extra, dict):
        for key in ("content", "refusal", "message"):
            text = normalize_model_text_content(model_extra.get(key))
            if text.strip():
                return text
    return ""


def message_content_has_image_part(content: object) -> bool:
    if not isinstance(content, list):
        return False
    return any(isinstance(part, dict) and part.get("type") == "image_url" for part in content)


def model_messages_have_images(messages: List[Dict[str, object]]) -> bool:
    return any(message_content_has_image_part(message.get("content")) for message in messages or [])


def chat_model_candidate_slots(messages: List[Dict[str, object]]) -> List[Tuple[str, Dict[str, object], str]]:
    try:
    ...
    for label, slot in (
        ("聊天模型", chat_slot),
        ("后台模型", model_slot_config(MODEL_SLOT_BACKGROUND)),
    ):
    ...
    if not any(str(slot.get("provider") or "").strip().lower() == "local" for _, slot, _ in candidates):
        ...
        if signature and signature not in seen:
            candidates.append(("本地兜底模型", local_slot, "chat model is not known vision-capable"))
    return candidates or [("聊天模型", chat_slot, "")]


def chat_trace_model_name(messages: List[Dict[str, object]]) -> str:
    candidates = chat_model_candidate_slots(messages)
    if candidates:
        return str(candidates[0][1].get("model") or MODEL_NAME)
    return str(model_slot_config(MODEL_SLOT_CHAT).get("model", MODEL_NAME))


def _iter_model_deltas_raw(
    ...
    candidates = [("本地开场", default_model_slot("local"), "")] if opening or force_local else chat_model_candidate_slots(messages)
    if recovery:
        candidates = [(label, {**slot, "thinking_enabled": False}, reason) for label, slot, reason in candidates]
    ...
    for index, (_slot_label, model_slot, _fallback_reason) in enumerate(candidates):
        ...
        try:
            ...
            last_exc = exc
            if yielded_any or index >= len(candidates) - 1 or not image_message_error_is_incompatible(exc):
                raise
            continue
        finally:
            http_client.close()
...
def call_chat_completion_once_with_slot(
    messages: List[Dict[str, object]],
    max_tokens: int,
    temperature: float,
    top_p: float,
    model_slot: Dict[str, object],
) -> str:
    client, http_client, _ = openai_client_for_model_slot_config(model_slot, timeout=REQUEST_TIMEOUT)
    try:
        resp = client.chat.completions.create(
...
def _call_chat_completion_once_raw(
    messages: List[Dict[str, object]],
    max_tokens: int,
    temperature: float,
    top_p: float,
    ...
    for index, (_slot_label, model_slot, _fallback_reason) in enumerate(candidates):
        try:
            return call_chat_completion_once_with_slot(messages, max_tokens, temperature, top_p, model_slot)
        except Exception as exc:
            last_exc = exc
            if index >= len(candidates) - 1 or not image_message_error_is_incompatible(exc):
                raise
            continue
    if last_exc is not None:
        raise last_exc
...
def _reply_recovery_factories(raw, messages, max_tokens, temperature, top_p, opening):
    instruction = {"role": "system", "content": "请只给出面向用户的最终答复，不输出自我分析、内部字段或推理过程。未执行的操作不得宣称成功。"}
    def attempt(index):
        current = messages
        if index > 0:
        ...
        result = raw(current, max_tokens, temperature, top_p, opening=opening,
                     force_local=index == 2, recovery=index > 0)
        return [result] if isinstance(result, str) else result
    return [lambda i=i: attempt(i) for i in range(3)]


def _record_reply_recovery(attempt, reason):
    # Deliberately exclude response text, prompt and credentials.
    record_event(None, "reply_internal_recovery", "", {"attempt": attempt, "reason": reason})


def iter_model_deltas(messages, max_tokens, temperature, top_p, opening=False):
    yield from recover_reply_stream(
        _reply_recovery_factories(_iter_model_deltas_raw, messages, max_tokens, temperature, top_p, opening),
        _record_reply_recovery)


def call_chat_completion_once(messages, max_tokens, temperature, top_p, opening=False):
    return "".join(recover_reply_stream(
        _reply_recovery_factories(_call_chat_completion_once_raw, messages, max_tokens, temperature, top_p, opening),
        _record_reply_recovery))
...
def visitor_ip(request: Request) -> str:
    ...
    device_id = clean_device_id(request.headers.get("x-wangcai-device-id", ""))
    if not device_id:
