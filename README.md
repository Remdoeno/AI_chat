...
旺财1.5 是一个面向家庭本地部署的 AI 助手网页端。它把本地大模型、长期记忆、浏览器设备身份、跨设备记忆绑定、联网搜索、多模态图片输入、后台空闲创作和管理员观察界面整合在一个 FastAPI 单体服务里。
...
- 认可并保留用户亲自调整的主 `SYSTEM_PROMPT`：后续工程修复只在独立系统约束或专门 agent prompt 中追加规则，避免误覆盖主 prompt。
- 修复模型偶尔把内部 `[message_time: ...]` 前缀复述到聊天气泡里的问题；前端会隐藏这类内部时间标记，系统约束也要求不要输出。
- 修复黑暗模式下聊天页和分析模式中“助手/你”标签颜色过暗的问题，黑金主题下发言者标签会使用更明亮的颜色。
- 记忆写入从字符串 leak 启发式升级为语义 validation agent：assistant 复述用户原话不再被误判为泄漏，assistant 脑补内容仍会被拒绝。
- 记忆整理 agent 增加 JSON 修复链路，并提高默认输出预算，减少后台任务因 JSON 截断而失败。
...
旺财1.5 默认每次打开网页仍会创建一个新 session，这样普通刷新不会直接把旧聊天搬回来。需要续接时：
