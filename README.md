...
![Wangcai 2.2 development journey](static/brand/wangcai-2.0-journey.png)

Wangcai 2.2 是一个面向家庭本地部署的 AI 助手网页端。它把本地/联网大模型、长期记忆、联网搜索、多模态图片输入、HiDream 文生图、固定角色库、图文成果库、后台空闲创作和管理员分析模式整合在一个 FastAPI Web 系统里。

2.2 是当前服务器稳定快照版：在 2.1 的固定角色库和成果小剧场基础上，进一步收紧成果导演指令、后台创作 prompt 优先级、连续作品规划、成果容错、发布边界和 7777 端口约定。它把线上 `wangcai_ai` 的稳定运行状态固定为公开版本，方便后续继续迭代。

2.1 的核心变化仍然是固定演员表的小剧场系统。你可以在角色库里创建、修改、补图和管理角色，让角色以全局索引的形式参与后续聊天、绘图和成果创作；成果库不再只围绕临时生成的陌生角色转，而可以持续调用已沉淀的角色设定、形象、性格和剧情指令。

2.0 引入的视觉创作链路仍然是 2.2 的基础：你可以在聊天里直接点“画图”，让旺财先把自然语言改写成适合图像模型的英文 prompt，再调用图像生成服务一次生成 4 张图，直接预览和下载；后台成果也升级为图文作品，每篇成果至少带一张主题配图，并在成果库里以封面图作为主要展示对象。
...
- 更重视图像生成：Wangcai 2.2 内置文生图工作流，支持 HiDream-O1-Image-Dev-2604 等本地图像服务；聊天和分析模式都可以切换到画图，本轮消息会先进行 prompt 翻译、分类和优化，再生成固定 4 张图片。专业 prompt 会尽量直译保留，自然语言描述会被扩展成更适合出图的视觉指令，连续修改会基于上一轮 optimized prompt 继续修正。
...
- 固定当前服务器 `wangcai_ai` 稳定快照为 2.2：本版本对应已拆分的 `wangcai_app` 主线，入口 `app.py` 保持兼容加载器，线上运行约定统一为 `7777`。
- 强化成果小剧场导演指令：聊天中的成果、剧情、角色出场和配图方向可以转化为隐藏导演指令，供后续 idle 成果生成参考。
- 强化后台创作 prompt 优先级：用户在成果页配置的创作 prompt 高于角色库、历史系列、近期避重复规则和长期导演指令；明确点名的地点、题材、角色、系列和事件必须执行。
- 新增连续作品规划约束：剧情规划 agent 会读取同一系列的主线、番外、前传、外传和起源篇，给出标题方向、剧情节拍、出场角色、场景计划和配图方向，减少系列漂移。
- 优化成果生成容错：后台创作模型输出损坏或不完整 JSON 时，系统会先尝试结构化修复，再用确定性兜底规则恢复标题、正文、摘要、类型和配图计划。
- 优化成果命名与篇数控制：当创作 prompt 明确要求某个系列和篇数时，保存阶段会优先采用规划结果中的 `series_title`、`episode_index` 和完结策略。
- 新增后台创作频率配置：成果库可以调整 idle agent 的基础运行间隔，减少后台创作过密或过慢的问题。
- 继续收紧发布边界：公开仓库只包含代码、静态资源、公开 README 和 release log；不包含生成图片、聊天数据库、成果数据库、记忆库、日志、密钥、代理或本地配置。
...
Wangcai 2.2 的画图模式不是简单把用户原话丢给图像模型，而是一条完整的生图链路：
...
Wangcai 2.2 默认每次打开网页仍会创建一个新 session，这样普通刷新不会直接把旧聊天搬回来。需要续接时：
...
2.2 的重点是把服务器当前稳定状态正式固定下来：成果导演指令、连续作品规划、prompt 优先级、成果容错、角色复用和发布边界都进入更可持续的状态。

简单说：Wangcai 2.2 是一个能记住你、陪你聊、帮你查、自己写图文作品，并且能让固定角色在长期小剧场里持续登场的本地 AI Web 系统。
