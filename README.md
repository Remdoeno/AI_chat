...
![Wangcai 2.0 development journey](static/brand/wangcai-2.0-journey.png)

Wangcai 2.0 是一个面向家庭本地部署的 AI 助手网页端。它把本地/联网大模型、长期记忆、联网搜索、多模态图片输入、HiDream 文生图、图文成果库、后台空闲创作和管理员分析模式整合在一个 FastAPI Web 系统里。

2.0 的核心变化是：旺财不再只是“会聊天、会记忆、会写东西”的助手，而是开始具备完整的视觉创作链路。你可以在聊天里直接点“画图”，让旺财先把你的自然语言改写成适合图像模型的英文 prompt，再调用图像生成服务一次生成 4 张图，直接预览和下载；后台成果也升级为图文作品，每篇成果至少带一张主题配图，并在成果库里以封面图作为主要展示对象。
...
- 更重视图像生成：Wangcai 2.0 内置文生图工作流，支持 HiDream-O1-Image-Dev-2604 等本地图像服务；聊天和分析模式都可以切换到画图，本轮消息会先进行 prompt 翻译、分类和优化，再生成固定 4 张图片。专业 prompt 会尽量直译保留，自然语言描述会被扩展成更适合出图的视觉指令，连续修改会基于上一轮 optimized prompt 继续修正。
...
Wangcai 2.0 的画图模式不是简单把用户原话丢给图像模型，而是一条完整的生图链路：
...
Wangcai 2.0 默认每次打开网页仍会创建一个新 session，这样普通刷新不会直接把旧聊天搬回来。需要续接时：
...
简单说：Wangcai 2.0 是一个能记住你、陪你聊、帮你查、自己写作品，并且能把文字想象变成图片的本地 AI Web 系统。
