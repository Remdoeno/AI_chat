...
# It exposes the Wangcai web app as http://127.0.0.1:19922/ and, with -g,
# also allows LAN clients to reach http://<this-mac-ip>:19922/.

SSH_KEY="${WANGCAI_SERVER_SSH_KEY:-$HOME/.ssh/icfc}"
SERVER_SSH_HOST="${WANGCAI_SERVER_SSH_HOST:-127.0.0.1}"
SERVER_SSH_PORT="${WANGCAI_SERVER_SSH_PORT:-10022}"
SERVER_USER="${WANGCAI_SERVER_USER:-root}"
LOCAL_WEB_PORT="${WANGCAI_LOCAL_WEB_PORT:-19922}"
LOCAL_TUNNEL_PORT="${WANGCAI_LOCAL_TUNNEL_PORT:-19923}"
REMOTE_WEB_HOST="${WANGCAI_REMOTE_WEB_HOST:-127.0.0.1}"
REMOTE_WEB_PORT="${WANGCAI_REMOTE_WEB_PORT:-9922}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROXY_SCRIPT="${WANGCAI_IP_PROXY_SCRIPT:-${SCRIPT_DIR}/wangcai_ip_proxy.py}"
LOG_DIR="${WANGCAI_TUNNEL_LOG_DIR:-${SCRIPT_DIR}/../log}"
PROXY_PID_FILE="${WANGCAI_IP_PROXY_PID_FILE:-${LOG_DIR}/wangcai_ip_proxy_${LOCAL_WEB_PORT}.pid}"
PROXY_LOG_FILE="${WANGCAI_IP_PROXY_LOG_FILE:-${LOG_DIR}/wangcai_ip_proxy_${LOCAL_WEB_PORT}.log}"
AUTO_BIND_HOST="$(ifconfig 2>/dev/null | awk '/inet / && $2 != "127.0.0.1" && $2 !~ /^169[.]254[.]/ {print $2; exit}')"
if [ -n "${WANGCAI_LOCAL_BIND_HOSTS:-}" ]; then
  LOCAL_BIND_HOSTS="${WANGCAI_LOCAL_BIND_HOSTS}"
...
if ! ssh -i "${SSH_KEY}" \
  ...
  printf '%s\n' "Server SSH or wangcai_ai health check failed."
...
printf '%s\n' "Wangcai web ready: http://127.0.0.1:${LOCAL_WEB_PORT}/"
printf '%s\n' "Bind hosts: ${LOCAL_BIND_HOSTS}"
